...
export const signUp = catchAsync(async (req, res, next) => {
  ...
  if (req.body.firstPhone === "" || req.body.firstPhone.length < 10) {
    return res.status(409).json({ message: err_phone2 });
  }
  if (req.body.email !== "") {
    let existUser = await userModel.findOne({ firstPhone: req.body.firstPhone });
...
export const vierfyVerificationCode = catchAsync(async (req, res, next) => {
  ...
  if (req.body.verificationCode !== "") {
    ...
    let userData = await userModel.findOne({ verificationCode, _id: req.userId });
    if (userData) {
